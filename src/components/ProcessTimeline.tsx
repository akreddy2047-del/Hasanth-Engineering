...
export default function ProcessTimeline() {
  ...
  return (
    <section 
      id="process" 
      className="py-16 bg-white font-sans border-b border-[#e2e8f0] scroll-mt-20"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {stepsList.map((st, idx) => (
            <div 
              key={idx}
              className="bg-white border border-[#e2e8f0] rounded p-6 flex flex-col justify-between hover:border-[#0056b3] transition-colors duration-150"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="text-3xl font-sans text-[#0056b3]/20">
                    {st.stepNum}
                  </span>
                  <span className="text-[10px] font-sans uppercase text-[#1e293b] tracking-wider">
                    Phase {st.stepNum}
                  </span>
                </div>
                ...
                <h3 className="text-lg font-sans text-[#1e293b] uppercase tracking-tight mt-1">
                  {st.title}
                </h3>
                
                <p className="text-xs text-[#1e293b] mt-3 leading-relaxed">
                  {st.desc}
                </p>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-[#e2e8f0]">
                <ul className="space-y-1.5">
                  {st.milestones.map((mi, mIdx) => (
                    <li key={mIdx} className="flex items-center gap-2 text-[11px] text-[#1e293b]">
